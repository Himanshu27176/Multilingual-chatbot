...
applyTheme();
applyFontSize(localStorage.getItem('lb_font') || 'medium');
...
function applyTheme(mode) {
  ...
  document.getElementById('theme-dark')?.classList.toggle('active', isDarkMode);
  document.getElementById('theme-light')?.classList.toggle('active', !isDarkMode);
}
...
function applyFontSize(size) {
  ...
  localStorage.setItem('lb_font', size);
...
document.getElementById('btn-settings').addEventListener('click', () => {
  ...
  applyTheme(); // refresh theme buttons
...
document.getElementById('btn-save-username').addEventListener('click', () => {
  ...
  if (!/^[a-zA-Z0-9_]{3,50}$/.test(newName)) return showToast('⚠️ 3-50 alphanumeric characters only');
...
document.getElementById('btn-delete-account').addEventListener('click', async () => {
  ...
  if (!confirm('This will delete ALL your chats and data permanently. Continue?')) return;
  showToast('🗑️ Account deleted. Goodbye!');
  setTimeout(() => {
...
function detectLanguage(text){for(const{code,regex}of LANG_DETECT_MAP){if(regex.test(text))return code;}return null;}

msgInput.addEventListener('input',()=>{
  autoResize(msgInput);
  const text=msgInput.value;
  if(text.length>5){const detected=detectLanguage(text);if(detected&&detected!==currentLang){currentLang=detected;chatLangSelect.value=currentLang;updateLangBadge();showLangToast(detected);}}
});
...
// ── SESSIONS ─────────────────────────────────────────────────
async function loadSessions(){try{sessions=await apiFetch('/chat/sessions','GET');renderSessions();}catch(e){}}
function renderSessions(){
  if(!sessions.length){sessionsList.innerHTML='<p class="sessions-empty">No chats yet. Start one!</p>';return;}
  sessionsList.innerHTML=sessions.map(s=>`<div class="session-item ${currentSession===s.id?'active':''}" data-id="${s.id}" onclick="loadSession(${s.id})"><span class="s-title">${escHtml(s.title)}</span><span class="s-lang">${(s.language||'en').toUpperCase()}</span><button class="s-del" onclick="deleteSession(event,${s.id})" title="Delete">✕</button></div>`).join('');
}
async function loadSession(id){
  currentSession=id; const sess=sessions.find(s=>s.id===id);
  ...
  try{const msgs=await apiFetch('/chat/messages/'+id,'GET');msgs.forEach(m=>appendMessage(m.role==='user'?'user':'bot',m.content,false));scrollToBottom();}catch(e){}
  closeSidebar();
}
...
function resetChat(){
  ...
  Array.from(messagesArea.children).forEach(c=>{if(c!==welcomeScreen)c.remove();});
  welcomeScreen.style.display='flex'; renderSessions(); clearImagePreview();
}
...
async function sendMessage(customText){
  ...
  if(!customText){msgInput.value='';autoResize(msgInput);}
  welcomeScreen.style.display='none';
  if(image){appendImageMessage('user',text,image.previewUrl);clearImagePreview();}
  else{appendMessage('user',text);}
  const typing=showTyping();
  try{
    let res;
    if(image){res=await apiFetch('/chat/image','POST',{message:text||'Analyze this image. Detect text, identify language, translate if needed, and describe what you see.',language:currentLang,session_id:currentSession||undefined,image:image.base64,mime_type:image.mimeType});}
    else{res=await apiFetch('/chat/message','POST',{message:text,language:currentLang,session_id:currentSession||undefined});}
    removeTyping(typing); appendMessageStream('bot',res.reply);
    if(!currentSession){currentSession=res.session_id;chatTitle.textContent=(text||'📷 Image').slice(0,50);await loadSessions();}
  }catch(err){removeTyping(typing);appendMessage('bot','⚠️ Error: '+(err.message||'Server error.'));}
}
...
document.getElementById('btn-do-translate').addEventListener('click',async()=>{
  ...
  if(!text)return;
  result.textContent='Translating…'; actions.classList.add('hidden');
  try{
    const res=await apiFetch('/chat/translate','POST',{text,target_language:target});
    result.textContent=res.translation;
    lastTranslation=res.translation;
    actions.classList.remove('hidden'); // show Send to Chat + Copy buttons
...
document.getElementById('btn-send-to-chat').addEventListener('click',()=>{
  ...
  lastTranslation='';
  showToast('💬 Translation sent to chat!');
});

// Copy translation
document.getElementById('btn-copy-translation').addEventListener('click',()=>{
  if(!lastTranslation)return;
  navigator.clipboard.writeText(lastTranslation).then(()=>showToast('📋 Translation copied!'));
});

// ── CAMERA ────────────────────────────────────────────────────
document.getElementById('btn-camera').addEventListener('click',()=>{document.getElementById('camera-modal').classList.remove('hidden');startCamera();});
document.getElementById('btn-camera-close').addEventListener('click',()=>{stopCamera();document.getElementById('camera-modal').classList.add('hidden');});
...
document.getElementById('file-input').addEventListener('change',function(){
  ...
  reader.onload=e=>{const b64Full=e.target.result;setPendingImage(b64Full.split(',')[1],file.type,b64Full);};
...
function startCamera(){
  ...
  navigator.mediaDevices.getUserMedia({video:{facingMode:'environment'}})
    ...
    .catch(()=>{showToast('Camera not available. Use upload instead.');document.getElementById('camera-modal').classList.add('hidden');});
}
...
document.getElementById('btn-snap').addEventListener('click',()=>{
  ...
  stopCamera(); video.classList.add('hidden'); document.getElementById('btn-snap').classList.add('hidden');
  document.getElementById('btn-retry').classList.remove('hidden');
  const dataUrl=canvas.toDataURL('image/jpeg',0.85);
  const prev=document.getElementById('camera-preview-img'); prev.src=dataUrl;
  document.getElementById('camera-preview-wrap').classList.remove('hidden');
  document.getElementById('btn-use-photo').dataset.base64=dataUrl.split(',')[1];
  document.getElementById('btn-use-photo').dataset.preview=dataUrl;
});
document.getElementById('btn-retry').addEventListener('click',()=>{document.getElementById('camera-preview-wrap').classList.add('hidden');startCamera();});
document.getElementById('btn-use-photo').addEventListener('click',function(){
  setPendingImage(this.dataset.base64,'image/jpeg',this.dataset.preview);
...
function setPendingImage(base64,mimeType,previewUrl){
  ...
  document.getElementById('image-preview-thumb').src=previewUrl;
  document.getElementById('image-preview-wrap').classList.remove('hidden');
  msgInput.placeholder='Ask about this image… (or send to analyze)';
  showToast('📷 Image ready!');
}
document.getElementById('btn-remove-image').addEventListener('click',clearImagePreview);
function clearImagePreview(){pendingImage=null;document.getElementById('image-preview-wrap').classList.add('hidden');msgInput.placeholder='Type or speak your message…';}

// ── MESSAGE RENDERING ─────────────────────────────────────────
function appendImageMessage(role,text,imgUrl){
  const div=document.createElement('div'); div.className='msg '+role;
  const time=new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});
  div.innerHTML=`<div class="msg-avatar">👤</div><div class="msg-body"><div class="msg-bubble"><img src="${imgUrl}" class="msg-image" alt="Sent image"/>${text?`<p style="margin-top:8px">${escHtml(text)}</p>`:''}</div><div class="msg-actions"><span class="msg-meta">${time}</span><div class="msg-btns"><button class="msg-action-btn" onclick="copyMsg(this)" title="Copy">📋</button><button class="msg-action-btn react-btn" onclick="reactMsg(this,'👍')">👍</button><button class="msg-action-btn react-btn" onclick="reactMsg(this,'👎')">👎</button></div></div></div>`;
  messagesArea.appendChild(div); scrollToBottom();
}
function appendMessageStream(role,text){
  const div=createMsgEl(role,''); messagesArea.appendChild(div); scrollToBottom();
  const bubble=div.querySelector('.msg-bubble'); const words=text.split(' '); let i=0;
  const iv=setInterval(()=>{if(i<words.length){bubble.innerHTML=escHtml(words.slice(0,i+1).join(' '))+'<span class="cursor">▋</span>';i++;scrollToBottom();}else{bubble.innerHTML=escHtml(text);clearInterval(iv);}},30);
}
function appendMessage(role,text,scroll=true){const div=createMsgEl(role,text);messagesArea.appendChild(div);if(scroll)scrollToBottom();}
function createMsgEl(role,text){
  const div=document.createElement('div'); div.className='msg '+role;
  const time=new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});
  const isBot=role==='bot';
  div.innerHTML=`<div class="msg-avatar">${isBot?'🤖':'👤'}</div><div class="msg-body"><div class="msg-bubble">${escHtml(text)}</div><div class="msg-actions"><span class="msg-meta">${time}</span><div class="msg-btns"><button class="msg-action-btn" onclick="copyMsg(this)" title="Copy">📋</button>${isBot?`<button class="msg-action-btn" onclick="speakMsg(this)" title="Read aloud">🔊</button>`:''}<button class="msg-action-btn react-btn" onclick="reactMsg(this,'👍')">👍</button><button class="msg-action-btn react-btn" onclick="reactMsg(this,'👎')">👎</button></div></div></div>`;
  return div;
}

// ── MESSAGE ACTIONS ───────────────────────────────────────────
window.copyMsg=function(btn){const bubble=btn.closest('.msg-body').querySelector('.msg-bubble');navigator.clipboard.writeText(bubble.innerText).then(()=>{btn.textContent='✅';setTimeout(()=>btn.textContent='📋',1500);});};
window.speakMsg=function(btn){const bubble=btn.closest('.msg-body').querySelector('.msg-bubble');speakText(bubble.innerText,currentLang);btn.textContent='🔈';setTimeout(()=>btn.textContent='🔊',2000);};
window.reactMsg=function(btn,emoji){const siblings=btn.closest('.msg-btns').querySelectorAll('.react-btn');siblings.forEach(b=>b.classList.remove('reacted'));btn.classList.toggle('reacted');showToast(btn.classList.contains('reacted')?emoji+' Reaction added':'Reaction removed');};
...
function showTyping(){const div=document.createElement('div');div.className='msg bot';div.innerHTML='<div class="msg-avatar">🤖</div><div class="msg-body"><div class="msg-bubble"><div class="typing-dots"><span></span><span></span><span></span></div></div></div>';messagesArea.appendChild(div);scrollToBottom();return div;}
...
document.getElementById('btn-voice').addEventListener('click',()=>{
  ...
  recognition=new SpeechRecognition(); recognition.lang=LANG_CODES[currentLang]||'en-US'; recognition.continuous=false; recognition.interimResults=true;
...
function speakText(text,lang){if(!window.speechSynthesis)return;window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.lang=LANG_CODES[lang]||'en-US';u.rate=0.95;window.speechSynthesis.speak(u);}

// ── EXPORT ────────────────────────────────────────────────────
document.getElementById('btn-export').addEventListener('click',()=>{
  const msgs=messagesArea.querySelectorAll('.msg'); if(!msgs.length)return showToast('No messages to export!');
  ...
  const blob=new Blob([content],{type:'text/plain;charset=utf-8'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='lingogo-'+Date.now()+'.txt';a.click();URL.revokeObjectURL(url);showToast('📄 Chat exported!');
});
document.getElementById('btn-export-pdf').addEventListener('click',()=>{
  const msgs=messagesArea.querySelectorAll('.msg'); if(!msgs.length)return showToast('No messages!');
