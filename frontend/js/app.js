...
document.getElementById('reg-username').addEventListener('input', e => {
  ...
  if (!/^[a-zA-Z0-9_]{3,50}$/.test(v)) { hint.style.color='var(--danger)'; hint.textContent='3-50 alphanumeric characters or underscore'; }
...
chatLangSelect.addEventListener('change', () => {
  currentLang = chatLangSelect.value; updateLangBadge();
});
...
msgInput.addEventListener('input', () => {
  ...
  if (text.length > 5) {
    ...
    if (detected && detected !== currentLang) {
      currentLang = detected; chatLangSelect.value = currentLang; updateLangBadge();
...
function showLangToast(lang) {
  ...
  if (!t) { t=document.createElement('div'); t.id='lang-toast'; t.style.cssText='position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:var(--accent);color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:999;transition:opacity .3s;'; document.body.appendChild(t); }
...
function appendMessageStream(role, text) {
  ...
  div.innerHTML = `<div class="msg-avatar">${role==='user'?'👤':'🤖'}</div>
    <div class="msg-body"><div class="msg-bubble" id="sb"></div><div class="msg-meta">${time}</div></div>`;
  messagesArea.appendChild(div); scrollToBottom();
  const bubble = div.querySelector('#sb'); bubble.removeAttribute('id');
...
function appendMessage(role, text, scroll=true) {
  ...
  const time = new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});
  div.innerHTML = `<div class="msg-avatar">${role==='user'?'👤':'🤖'}</div>
    <div class="msg-body"><div class="msg-bubble">${escHtml(text)}</div><div class="msg-meta">${time}</div></div>`;
...
document.getElementById('btn-voice').addEventListener('click', () => {
  if (!SpeechRecognition) { alert('Voice not supported. Use Chrome/Edge.'); return; }
...
function speakText(text, lang) {
  ...
  u.lang = LANG_CODES[lang]||'en-US'; u.rate=0.95;
...
document.getElementById('btn-export').addEventListener('click', () => {
  ...
  msgs.forEach(msg => {
    ...
    const bubble=msg.querySelector('.msg-bubble'); const meta=msg.querySelector('.msg-meta');
    content+='['+(meta?meta.innerText:'')+'] '+role+':\n'+(bubble?bubble.innerText:'')+'\n\n';
  });
